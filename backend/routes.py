...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        session_id = request.session_id or generate_session_id("chat", request)
        ...
        try:
            ...
            is_new_session = (
                ...
                or not current_state.values.get("messages")
            )
        except Exception:
            is_new_session = True

        memory_context = (
            memory_manager.get_user_context(user_id) if is_new_session else None
        )
        ...
        return ChatResponse(response=extract_response_text(result))
    ...
    async def generate():
        try:
            ...
            error_msg = json.dumps(
                {"error": f"LLM 處理錯誤: {str(e)}"}, ensure_ascii=False
            )
...
async def analyze_tongue(request: TongueAnalysisRequest):
    ...
    try:
        ...
        user_id = request.user_id or "default"
        session_id = request.session_id or generate_session_id("tongue", request)
        config = create_agent_config(session_id)
    ...
    async def generate():
        try:
            ...
            config = create_agent_config(
                generate_session_id("tongue-stream", request)
            )
            ...
            error_msg = json.dumps(
                {"error": f"舌診分析錯誤: {str(e)}"}, ensure_ascii=False
            )
            yield f"data: {error_msg}\n\n"
...
async def predict_tongue_image(
    ...
    if _vision_loader is None or not _vision_loader.status.is_available:
        error_detail = (
            _vision_loader.status.error_message
            if _vision_loader
            else "vision-predict 模組未初始化"
        )
        raise HTTPException(
            status_code=503,
            detail=(
                f"vision-predict 模組不可用: {error_detail}. "
                f"請確保 vision-predict 目錄存在於 {settings.vision_predict_path} 且已安裝所有依賴項。"
            ),
        )
    ...
    async def generate():
        ...
        try:
            ...
            try:
                with tempfile.TemporaryDirectory() as tmp_output_dir:
                    if _vision_loader.status.analyze_function is None:
                        yield f"data: {json.dumps({'error': 'vision-predict 分析函數不可用'}, ensure_ascii=False)}\n\n"
                        return
                    ...
                    if prediction_results is None:
                        yield f"data: {json.dumps({'error': '圖片預測失敗，請檢查圖片格式和內容'}, ensure_ascii=False)}\n\n"
                        return
                    ...
                    session_id_value = session_id or generate_session_id(
                        "tongue-predict-stream", file
                    )
                    ...
                    config = create_agent_config(session_id_value)
            ...
            error_msg = json.dumps(
                {"error": f"處理過程發生錯誤: {str(e)}"}, ensure_ascii=False
            )
            yield f"data: {error_msg}\n\n"
...
async def agent_chat_stream(
    ...
    async def generate():
        ...
        try:
            ...
            session_id_value = session_id or generate_session_id(
                "agent-chat-stream", file if file else prompt
            )
            user_id_value = user_id or "default"
            ...
            if image_path:
                user_message = (
                    f"{prompt or '請幫我分析這張舌診圖片'}\n\n圖片已上傳，路徑為: {image_path}"
                )
                messages.append(HumanMessage(content=user_message))
            elif prompt:
                messages.append(HumanMessage(content=prompt))
            else:
                yield f"data: {json.dumps({'error': '請提供圖片或文字提示'}, ensure_ascii=False)}\n\n"
                return
...
async def get_user_context(user_id: str):
    ...
    try:
        ...
        return {"success": True, "context": context, "summary": summary}
    except Exception as e:
